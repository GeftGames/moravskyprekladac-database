...
if (file_exists($currenteditor)) {
    ...
    ob_get_clean();
} else echo "ERROR: Editor not found! '".$currenteditor.'"'; 
?>
<!DOCTYPE html>
<html lang="cs">
<head>
    ...
    <div id="content">
        ...
        <?php 
            $tabs=[
                [
                    ["source", "Podklad CS"],
                    [["noun", "Podstatná jména"], ["adverb", "Přídavná jména"],  ["pronoun", "Zájmena"],  ["number", "Číslovky"],  ["verb", "Slovesa"], ["preposition", "Předložky"]]
                ],
                [
                    ["global", "Globální"],
                ...
                [
                    ["simple", "Základní"],
                    [["sentence", "Věty"], ["sentencepart", "Část věty"], ["phrase", "Fráze"], ["simplewords", "Slova"], "|", ["sentencepattern", "Vět. vzorec"], ["phrasepattern", "Fráz. vzorec"]]
                ],
                [
                    ["translate", "Překlad"],
                    [["noun", "Podstatná jména"], ["adverb", "Přídavná jména"], ["pronoun", "Zájmena"], ["number", "Číslovky"], ["verb", "Slovesa"], ["adverb", "Příslovce"], ["preposition", "Předložky"], ["conjunction", "Spojky"], ["particle", "Částice"], ["", "Citoslovce"]]
                ]
            ];
            ...
            foreach ($tabs as $item) {
                ...
                $style="";
                if ($headerCode!=$selectPage) $style=" style='display: none'";
                else $headerEditor="<span>".$headerName."<span>";
                $tabsHTML.="<div id='tabsOption_$headerCode'$style>";

                $headerHTML.='<span id="mainOption_'.$headerCode.'" class="choice" onclick="selectMainOption('."'$headerCode'".')">'.$headerName.'</span>';
                ...
                foreach ($tabsBtns as $tabsBtn) {
                    if ($tabsBtn=="|") $tabsHTML.="|";
                        ...
                        $btnText=$tabsBtn[1];
                        $btnName=$tabsBtn[0];
                        $currentPage=$btnName==$selectEditor ? " selected" : "";
                        if ($btnName==$selectEditor && $headerCode==$selectPage) $headerEditor.="&gt;<span>".$btnText."</span>";
                        $tabsHTML.="<a class='tab$currentPage' href='./?page=$headerCode&editor=$btnName'>$btnText</a>";
                    }
                }
                $tabsHTML.="</div>";
            }
